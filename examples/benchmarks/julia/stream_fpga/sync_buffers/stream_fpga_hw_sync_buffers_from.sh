...
num_runs=100
output_file="results/stream_fpga_hw_sync_buffers_from.csv"

mkdir -p "results"
echo "sep=," > $output_file
echo "xclbin,kernel,allocation,syncto,run,verify,total" >> $output_file
...
do
    ...
    result=$(julia stream_fpga_hw_sync_buffers_from.jl)
