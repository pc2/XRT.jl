...
int main(int argc, char** argv) {
    ...
    xrtDeviceLoadXclbinFile(device, "/dev/shm/communication_PCIE.xclbin");
    uuid_t uuid;
    ...
    xrtKernelHandle kernel = xrtPLKernelOpen(device, uuid, "dummyKernel");
    ...
    char* a = NULL;
    posix_memalign((void**)&a, 4096, ARRAY_SIZE * sizeof(char));

    for (int i = 0; i < ARRAY_SIZE; i++) {
        a[i] = '0';
    }

    xrtBufferHandle abo = xrtBOAllocUserPtr(device, a, ARRAY_SIZE * sizeof(char), XRT_BO_FLAGS_NONE, xrtKernelArgGroupId(kernel, 0));
    timer_stop(&allocation_timer);

    // Sync input buffer to device
    Timer syncto_timer;
    timer_start(&syncto_timer);
    xrtBOSync(abo, XCL_BO_SYNC_BO_TO_DEVICE, ARRAY_SIZE * sizeof(char), 0);
    timer_stop(&syncto_timer);

    // Execute the kernel
    Timer run_timer;
    timer_start(&run_timer);
    xrtRunHandle run = xrtKernelRun(kernel, abo, '1', ARRAY_SIZE);
    xrtRunWait(run);
    ...
    xrtBOSync(abo, XCL_BO_SYNC_BO_FROM_DEVICE, ARRAY_SIZE * sizeof(char), 0);
