...
num_runs=100
output_file="results/stream_fpga_hw_sync_buffers_to.csv"

mkdir -p "results"
echo "sep=," > $output_file
echo "xclbin,kernel,allocation,run,syncfrom,verify,total" >> $output_file
...
do
    ...
    result=$(julia stream_fpga_hw_sync_buffers_to.jl)
