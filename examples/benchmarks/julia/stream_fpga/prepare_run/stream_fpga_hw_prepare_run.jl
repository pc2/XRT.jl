execution_time = @elapsed begin
    ...
    loadxclbinkernel_time = @elapsed begin
        # Create kernel und typed Run object
        kernel, uuid = @eval @prepare_run("/dev/shm/stream.xclbin", "stream_calc:{k1}", device=XRT.device(1))
    end
    ...
    # Execute the kernel
    run_time = @elapsed wait(Run_stream_calc__k1(xa, xb, xc, 2.0, UInt32(array_size), UInt32(1)))
