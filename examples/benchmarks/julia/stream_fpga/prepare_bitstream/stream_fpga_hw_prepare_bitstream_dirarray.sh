...
num_runs=100
output_file="results/stream_fpga_hw_prepare_bitstream_dirarray.csv"

mkdir -p "results"
echo "sep=," > $output_file
echo "prepare,allocate,run,verify,total" >> $output_file
...
do
    ...
    result=$(julia stream_fpga_hw_prepare_bitstream_dirarray.jl)
