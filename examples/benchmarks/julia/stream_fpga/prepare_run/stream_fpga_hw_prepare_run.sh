...
num_runs=100
output_file="results/stream_fpga_hw_prepare_run.csv"

mkdir -p "results"
echo "sep=," > $output_file
echo "prepare,allocate,syncto,run,syncfrom,verify,total" >> $output_file
...
do
    ...
    result=$(julia stream_fpga_hw_prepare_run.jl)
